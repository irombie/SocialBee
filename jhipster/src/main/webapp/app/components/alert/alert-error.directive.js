(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('stajProjeApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-stajProjeApp-error');
                var entityKey = httpResponse.headers('X-stajProjeApp-params');
                if (errorHeader) {
